...
with st.container(border=True):
    ...
    if cestas:
        ...
        # Verifica se o usuário selecionou uma cesta específica vindo da Home
        cesta_inicial_index = 0
        if "cesta_selecionada_home" in st.session_state and st.session_state["cesta_selecionada_home"]:
            id_home = st.session_state["cesta_selecionada_home"]
            for idx, item in enumerate(opcoes_cestas):
                if item.get("id") == id_home:
