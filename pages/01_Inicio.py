...
def promocao_ativa(cesta):
    """Mesma regra usada em app.py: promoção só vale dentro do período configurado."""
    if not cesta or not cesta.get("em_promocao") or cesta.get("preco_promocional") is None:
        return False
    try:
        agora = datetime.now()
        inicio = datetime.fromisoformat(str(cesta.get("promocao_inicio"))) if cesta.get("promocao_inicio") else None
        fim = datetime.fromisoformat(str(cesta.get("promocao_fim"))) if cesta.get("promocao_fim") else None
...
def preco_efetivo_cesta(cesta):
    """Preço que o cliente realmente paga: promocional se a promoção estiver ativa, senão o preço normal."""
    if not cesta:
        return 0.0
    if promocao_ativa(cesta):
        try:
            return float(cesta.get("preco_promocional"))
        except Exception:
            pass
    try:
        return float(cesta.get("preco", 0) or 0)
    except Exception:
        return 0.0
...
# ==========================================================
st.set_page_config(page_title="Finalizar Pedido | Doce Cesta Brasília", page_icon="🎁", layout="centered", initial_sidebar_state="collapsed")

# ==========================================================
# CACHING DINÂMICO UNIFICADO E OTIMIZADO (SEM CONGELAMENTOS)
# ==========================================================
...
def obter_secoes_e_cestas_ativas():
    try:
        ...
        if not secoes_ativas: secoes_ativas = ["Cestas de Café"]
...
# CACHE DE ALTA PERFORMANCE PARA O VIA CEP (Evita travar a UI)
...
.destaque-cesta-nome-local { font-family: 'Dancing Script', cursive !important; font-size: 38px !important; font-weight: 700 !important; color: #c5721f !important; line-height: 1.1 !important; margin-bottom: 2px; }

input, textarea, select { border-radius: 12px !important; border: 1px solid #dfcdbb !important; background-color: #faf7f3 !important; font-size: 14px !important; padding: 12px !important; }
input:focus, textarea:focus, select:focus { border-color: #c5721f !important; background-color: #ffffff !important; box-shadow: 0 0 0 3px rgba(197, 114, 31, 0.1) !important; }
div[data-testid="stCheckbox"] { background: #faf7f3; border: 1px solid #e8ddd3; padding: 12px 16px; border-radius: 14px; margin-bottom: 8px; display: flex; align-items: center; }

.receipt-box {
    background: #ffffff; background-image: radial-gradient(#faf7f3 20%, transparent 20%), radial-gradient(#faf7f3 20%, transparent 20%);
    background-position: 0 0, 10px 10px; background-size: 20px 20px; border: 1px solid #e8ddd3; border-radius: 16px; padding: 25px; margin-top: 10px;
}
...
.stButton button { background: linear-gradient(135deg, #137333 0%, #0d4e22) !important; color: white !important; border-radius: 16px !important; height: 60px !important; font-size: 17px !important; font-weight: 800 !important; border: none !important; box-shadow: 0 8px 25px rgba(19, 115, 51, 0.3) !important; text-transform: uppercase; margin-top: 15px; }
.sucesso-container { background: linear-gradient(145deg, #f0f7f4 0%, #e6f4ed 100%); border: 2px solid #137333; border-radius: 24px; padding: 40px 30px; text-align: center; margin-top: 20px; box-shadow: 0 15px 40px rgba(19, 115, 51, 0.15); }
...
def renderizar_passo(numero, titulo):
    st.markdown(f'<div class="step-container"><div class="step-number">{numero}</div><h3 class="step-title">{titulo}</h3></div>', unsafe_allow_html=True)
...
with st.container(border=True):
    ...
    with col_ddi: st.selectbox("DDI *", ["🇧🇷 +55", "🇺🇸 +1", "🇵🇹 +351", "🇪🇸 +34"], key="input_ddi_comprador")
    with col_tel: st.text_input("Seu WhatsApp *", placeholder="(61) 99999-9999", key="input_tel_comprador")
    with col_cpf: st.text_input("Seu CPF *", placeholder="Apenas números", max_chars=14, key="input_cpf_comprador", help="Necessário para segurança e emissão de recibo.")
...
if cestas_ativas and secoes_disponiveis:
    ...
    with st.container(border=True):
        ...
        def ao_mudar_secao(): st.session_state["cesta_selecionada_id"] = None

        if len(secoes_disponiveis) > 1:
            col_categoria, col_modelo = st.columns(2)
            with col_categoria:
                st.selectbox("💌 1. O que você deseja enviar?", secoes_disponiveis, index=secoes_disponiveis.index(st.session_state["secao_form"]) if st.session_state["secao_form"] in secoes_disponiveis else 0, key="secao_form", on_change=ao_mudar_secao)

            cestas_da_secao = [c for c in cestas_ativas if (c.get("secao_vitrine") or "Cestas de Café").strip().lower() == str(st.session_state["secao_form"]).strip().lower()]
            ...
            if st.session_state.get("cesta_selecionada_id"):
                for i, c in enumerate(opcoes_cestas):
                    if c["id"] == st.session_state["cesta_selecionada_id"]: cesta_idx = i; break

            with col_modelo:
                cesta_selecionada = st.selectbox("💝 2. Escolha o modelo", opcoes_cestas, format_func=lambda c: c["nome"], index=cesta_idx)
        else:
            cestas_da_secao = [c for c in cestas_ativas if (c.get("secao_vitrine") or "Cestas de Café").strip().lower() == str(st.session_state["secao_form"]).strip().lower()]
            opcoes_cestas = [{"id": None, "nome": "Clique para selecionar o modelo..."}] + cestas_da_secao
            cesta_idx = 0
            ...
            cesta_selecionada = st.selectbox("💝 Escolha o modelo do presente", opcoes_cestas, format_func=lambda c: c["nome"], index=cesta_idx)
        ...
        if cesta_selecionada and cesta_selecionada.get("id"):
            ...
            with col_img:
                ...
                if promocao_ativa(cesta_obj):
                    de_num = float(cesta_obj.get("preco", 0) or 0)
                    por_num = preco_efetivo_cesta(cesta_obj)
                    de_txt = f"R$ {de_num:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
                    por_txt = f"R$ {por_num:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
                    st.markdown(
                        f'<div style="margin-top: 15px; background: #ffffff; border: 2px solid #c5221f; border-radius: 16px; padding: 15px 10px; text-align: center;">'
                        f'<div style="font-size: 10px; font-weight: 800; color: #c5221f; text-transform: uppercase;">🔥 Promoção</div>'
                        f'<div style="font-size: 13px; color: #9a9a9a; text-decoration: line-through; font-weight: 600;">De {de_txt}</div>'
                        f'<div style="font-size: 28px; color: #c5221f; font-weight: 800; line-height: 1;">Por {por_txt}</div></div>',
                        unsafe_allow_html=True
                    )
                else:
                    valor_base_num = float(cesta_obj.get("preco", 0) or 0)
...
if polaroid:
    with st.container(border=True):
        ...
        if fotos_upload:
            if len(fotos_upload) > 2: st.error("⚠️ Limite excedido: Por favor, mantenha apenas 2 fotos anexadas.")
            else: st.success(f"✅ {len(fotos_upload)} foto(s) anexada(s) prontas para impressão!")
...
# ==========================================================
# PASSO 5: LOCAL E DATA DE ENTREGA (COM CACHE)
# ==========================================================
with st.container(border=True):
...
valor_base = preco_efetivo_cesta(cesta_obj) if cesta_obj else 0
...
linha_extras_html = f'<div class="receipt-line"><span>🎀 Mimos Extras</span> <strong>{valor_adc_fmt}</strong></div>' if valor_adicionais > 0 else ""
linha_cesta_nome = esc(cesta_obj['nome']) if cesta_obj else ""
if cesta_obj and promocao_ativa(cesta_obj):
    linha_cesta_nome += " <span style='font-size:11px; color:#c5221f; font-weight:800;'>🔥 PROMOÇÃO</span>"
...
<div class="receipt-line"><span>🎁 <b>{linha_cesta_nome}</b></span> <strong>{valor_base_fmt}</strong></div>
{linha_extras_html}
...
if enviar:
    ...
    dados = {
        ...
        "produtos": "\n".join(produtos_txt),
        "adicionais": texto_adicionais_bd, # As fotos agora usam a tabela certa e não texto
        "pagamento": pagamento,
    ...
    with st.spinner("Reservando seu presente e finalizando pedido..."):
        ...
        if sucesso:
            # 1. Salva os Adicionais se houver
            if adicionais_selecionados: 
                salvar_adicionais_pedido(pedido_id, adicionais_selecionados)
            
            # 2. Agora o Pedido EXISTE. Podemos atrelar as fotos físicas com segurança à tabela correta!
