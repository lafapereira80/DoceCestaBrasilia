...
# NOVA MATEMÁTICA PROTEGIDA CONTRA O "SOB CONSULTA"
cesta_preco_bd = cesta_obj.get("preco") if cesta_obj else 0
valor_base = float(cesta_preco_bd) if cesta_preco_bd is not None else 0

valor_adicionais = sum([float(item["preco"]) for item in adicionais_selecionados if item["preco"] is not None])
tem_consulta = (cesta_preco_bd is None) or any(item["preco"] is None for item in adicionais_selecionados)
total_estimado = valor_base + valor_adicionais

# MÁGICA DA EXIBIÇÃO
if cesta_preco_bd is None:
    valor_base_fmt = "Sob Consulta"
    total_fmt = "A calcular (Sob Consulta)"
else:
    valor_base_fmt = f"R$ {valor_base:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
    if tem_consulta:
        total_fmt = f"R$ {total_estimado:,.2f} + Itens sob consulta".replace(",", "X").replace(".", ",").replace("X", ".")
    else:
...
""", unsafe_allow_html=True)
        if tem_consulta: st.warning("⚠️ **Nota:** O seu pedido possui itens '*Sob Consulta*'. O valor total exato será confirmado pela nossa equipe via WhatsApp.")
