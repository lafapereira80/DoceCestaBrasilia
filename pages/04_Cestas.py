...
if not cestas:
    ...
    for cesta in cestas:
        ...
        with st.container(border=True):
            ...
            with col1:
                if cesta.get("imagem"):
                    # Ajustado a proporção para acomodar o botão de exclusão
                    img_col, txt_col = st.columns([1.2, 3.8])
                    with img_col:
                        st.image(cesta["imagem"], width=60)
                        # BOTÃO PARA EXCLUIR APENAS A IMAGEM DA CESTA
                        if st.button("❌", key=f"rm_img_{cesta['id']}", help="Excluir foto desta cesta", use_container_width=True):
                            try:
                                remover_imagem_cesta(cesta["id"])
                                st.rerun()
                            except Exception as erro:
                                st.error(f"Erro ao remover foto: {erro}")
