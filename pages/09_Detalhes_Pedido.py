...
div[data-testid="stVerticalBlockBorderWrapper"] { 
    ...
    padding: 16px 20px !important; margin-bottom: 8px !important; box-shadow: 0 2px 8px rgba(0,0,0,0.02); transition: all 0.25s ease; 
}
...
# =====================================================
# FUNÇÕES E LÓGICAS (ATUALIZADO PARA EXTRAS DINÂMICOS)
# =====================================================
...
if pedido_arquivado:
    st.success("📦 **PEDIDO FINALIZADO (ARQUIVO)** - Esta ficha está disponível apenas para leitura.")
...
# Carrega os itens extras dinâmicos do banco
if "lista_extras_dinamicos" not in st.session_state:
    ...
    # Converte o dicionário do banco para a lista da interface
    lista_temp = []
    for nome, val in itens_salvos.items():
        if "Valor Manual de" not in nome:  # Filtra para não conflitar com itens de catálogo antigos
...
# =====================================================
    ...
    with col_esquerda:
        ...
        with c_p2:
            with st.container(border=True):
                st.markdown('<div class="card-title">🎀 Itens Adicionais (Catálogo)</div>', unsafe_allow_html=True)
                if adicionais_pedido:
                    for adicional in adicionais_pedido:
                        ...
                        if valor is not None:
                            st.markdown(f"<div style='font-size:13px; margin-bottom:4px; font-weight:600;'>➕ {nome}</div>", unsafe_allow_html=True)
                        else:
                            st.markdown(f"<div style='font-size:13px; margin-bottom:2px; font-weight:600;'>➕ {nome} <span style='color:#c5721f; font-size:10px;'>(Consulta)</span></div>", unsafe_allow_html=True)
                else: st.caption("Nenhum item do catálogo solicitado.")
    ...
    # -------------------------------------------------
    # COLUNA DIREITA: FINANCEIRO, FOTOS E EXTRAS
    # -------------------------------------------------
    with col_direita:
        valor_cesta = 0.0
        ...
        with st.container(border=True):
            ...
            horario_obj = st.time_input("🕒 Horário Fixo (Exato)", value=hora_padrao, disabled=travar_financeiro)
            horario_str_salvar = horario_obj.strftime("%H:%M")
            
            # --- BLOCO DOS EXTRAS AVULSOS DINÂMICOS ---
            ...
            with col_add3:
                ...
                if st.button("➕", disabled=travar_financeiro, help="Adicionar Extra"):
            ...
            with cf3: desconto = st.number_input("🏷️ Desc. (R$)", min_value=0.0, value=float(pedido.get("desconto") or 0), step=1.0, disabled=travar_financeiro)
            ...
            for ad in adicionais_pedido:
                if ad.get("valor_unitario") is not None:
                    valor_adicionais_catalogo += float(ad["valor_unitario"])
            ...
            st.markdown(
                ...
                <div class="resumo-container">
                    ...
                    <div class="resumo-row"><span class="resumo-label">➕ Extras Avulsos Dinâmicos</span><span class="resumo-val">{formatar_valor(valor_extras_total)}</span></div>
        ...
        with st.container(border=True):
            ...
            anotacao = st.text_area("", value=pedido.get("anotacoes_internas") or "", height=80, key="campo_anotacao", disabled=pedido_arquivado, label_visibility="collapsed")
    ...
    if not pedido_arquivado:
        with col_bot1:
            if st.button("💾 Salvar Ficha Completa", use_container_width=True, type="primary", disabled=bloquear_edicao):
                ...
                # Monta o JSON de Extras Dinâmicos para o banco
                json_extras_salvar = {}
    ...
    with col_bot2:
        link_wpp = gerar_whatsapp(pedido, adicionais_pedido, valor_total_calculado, valor_frete, st.session_state["lista_extras_dinamicos"], desconto)
