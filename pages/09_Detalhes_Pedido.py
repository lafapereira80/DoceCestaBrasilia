...
/* ==========================================
   ESTILIZAÇÃO DOS CONTAINERS NATIVOS (CARDS)
========================================== */
div[data-testid="stVerticalBlockBorderWrapper"] {
    ...
    margin-bottom: 12px;
    box-shadow: 0 2px 4px rgba(90, 59, 40, 0.04);
}
    ...
    with st.container(border=True):
        ...
        if nomes_cestas:
            if cesta_atual in nomes_cestas:
                indice_cesta = nomes_cestas.index(cesta_atual)
            else:
                indice_cesta = 0
        ...
        nova_mensagem = st.text_area("💌 Mensagem", value=pedido.get("mensagem", ""), height=80)
        novo_especial = st.text_area("✨ Pedido Especial", value=pedido.get("pedido_especial", ""), height=80)
        novo_endereco = st.text_area("📍 Endereço", value=pedido.get("endereco", ""), height=100)
...
with st.container(border=True):
    ...
    with col1:
        st.markdown(
            ...
            <div class="info-value">{pedido.get('cliente_nome','-')}</div>
            """,
            unsafe_allow_html=True
        )
    ...
    with col2:
        st.markdown(
            ...
            <div class="info-value">{pedido.get('cliente_cpf','-')}</div>
            """,
            unsafe_allow_html=True
        )
    ...
    with col3:
        st.markdown(
            ...
            <div class="info-value">{pedido.get('cliente_telefone','-')}</div>
            """,
            unsafe_allow_html=True
        )
...
with st.container(border=True):
    ...
    with col1:
        st.markdown(
            ...
            <div class="info-value">{pedido.get('cesta_nome','-')}</div>
            """,
            unsafe_allow_html=True
        )
    ...
    with col2:
        st.markdown(
            ...
            <div class="info-value">{pedido.get('pagamento','-')}</div>
            """,
            unsafe_allow_html=True
        )
    ...
    with col3:
        st.markdown(
            ...
            <div class="info-value">{formatar_data(pedido.get('data_entrega'))}</div>
            """,
            unsafe_allow_html=True
        )
    ...
    with col4:
        st.markdown(
            ...
            <div class="info-value">{pedido.get('periodo_entrega','-')}</div>
            """,
            unsafe_allow_html=True
        )
...
with col1:
    with st.container(border=True):
        ...
        if produtos:
            ...
            st.info("Nenhum produto informado.")
...
with col2:
    with st.container(border=True):
        ...
        if adicionais_pedido:
            ...
            st.info("Nenhum adicional selecionado.")
...
with col1:
    with st.container(border=True):
        st.markdown('<div class="card-title">🎀 Total Adicionais</div>', unsafe_allow_html=True)
        st.success(formatar_valor(valor_adicionais))

with col2:
    with st.container(border=True):
        st.markdown('<div class="card-title">⚠️ Valores Sob Consulta</div>', unsafe_allow_html=True)
        st.info(formatar_valor(valor_consulta))
...
with col2:
    with st.container(border=True):
        ...
        st.text_area(
            "",
            value=pedido.get("pedido_especial", ""),
            disabled=True,
            height=80,
            key="pedido_especial"
        )
...
with st.container(border=True):
    ...
    st.caption("Uso exclusivo da equipe.")

    anotacao = st.text_area(
        "Observações do atendimento",
        value=pedido.get("anotacoes_internas", "") or "",
        height=100,
        key="campo_anotacao"
    )
...
with st.container(border=True):
    ...
    st.caption("Valores sob consulta entram no total após definição.")
    ...
    with col2:
        desconto = st.number_input(
            "🏷️ Desconto",
            min_value=0.0,
            value=float(pedido.get("desconto", 0) or 0),
            step=1.0,
            key="desconto"
        )
    ...
    st.markdown('<div class="card-title" style="margin-top: 15px;">🕒 Horário da Entrega</div>', unsafe_allow_html=True)

    horario_combinado = st.text_input(
        "Horário combinado",
        value=pedido.get("horario_combinado", ""),
...
with st.container(border=True):
    ...
    st.markdown(
        ...
        <tr>
        <td><b>💰 TOTAL</b></td>
        <td align="right"><h3>{formatar_valor(valor_total_calculado)}</h3></td>
        </tr>
        </table>
        </div>
        """,
    ...
    st.write(f"💳 Pagamento: **{pedido.get('pagamento','-')}**")
...
if st.button("💾 Salvar Atendimento", use_container_width=True, type="primary"):
    dados = {
        "status": status,
