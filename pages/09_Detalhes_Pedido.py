...
/* Header Banner - Fonte Ajustada e Compacta */
.order-header {
    background: linear-gradient(135deg, #ffffff 0%, #fdfbf8 100%);
    padding: 16px 24px; border-radius: 14px; border: 1px solid #e8ddd3;
    box-shadow: 0 4px 15px rgba(90, 59, 40, 0.03); margin-bottom: 24px;
    display: flex; justify-content: space-between; align-items: center;
}
.order-id { font-size: 18px; font-weight: 800; color: #4a2e1b; margin: 0; display: flex; align-items: center; gap: 8px;}
.order-type-badge { background: #fef7e0; color: #b06000; padding: 4px 10px; border-radius: 6px; font-size: 10px; font-weight: 800; border: 1px solid #fce8b2; text-transform: uppercase; }
...
.status-value { font-size: 15px; font-weight: 800; color: #c5721f; text-align: right; }

/* Content Cards Superiores */
.info-card {
    background: #ffffff; border: 1px solid #e8ddd3; border-radius: 14px; padding: 20px;
    box-shadow: 0 4px 12px rgba(90, 59, 40, 0.02); margin-bottom: 18px; height: 100%;
}
.card-title { font-size: 13.5px; font-weight: 800; color: #c5721f; margin-bottom: 14px; display: flex; align-items: center; gap: 8px; border-bottom: 2px dashed #f5eee6; padding-bottom: 8px; text-transform: uppercase; }
...
/* Receipt (Corrigido para não vazar HTML) */
...
/* Containers nativos */
div[data-testid="stVerticalBlockBorderWrapper"] { background: #ffffff !important; border-radius: 14px !important; border: 1px solid #e8ddd3 !important; padding: 20px !important; box-shadow: 0 4px 12px rgba(90, 59, 40, 0.02) !important; margin-bottom: 15px !important; }
</style>
...
tipo_texto = "🏢 B2B" if is_b2b else "🛍️ VAREJO"
...
if "modo_edicao_nativo" not in st.session_state:
    st.session_state.modo_edicao_nativo = False
...
# =====================================================
# MODO DE VISUALIZAÇÃO E FECHAMENTO (PADRÃO)
# =====================================================
if not st.session_state.modo_edicao_nativo:
    ...
    with col_fin:
        with st.container(border=True):
            ...
            # Construção Limpa do HTML do Extrato (Evitando vazamento de tags)
            html_extrato = f"""
            <div class="receipt-box">
                <div class="receipt-line"><span>Total Atual do Pedido</span> <span>R$ {formata_moeda(total_atual_db)}</span></div>
            """
            if desconto_perc > 0:
                html_extrato += f'<div class="receipt-line" style="color: #b06000;"><span>Novo Desconto ({desconto_perc}%)</span> <span>- R$ {formata_moeda(v_desconto_novo)}</span></div>'
            if novo_frete > 0:
                html_extrato += f'<div class="receipt-line"><span>Novo Frete/Taxa</span> <span>+ R$ {formata_moeda(novo_frete)}</span></div>'
            if v_extra_novo > 0:
                html_extrato += f'<div class="receipt-line"><span>Novo Item Extra</span> <span>+ R$ {formata_moeda(v_extra_novo)}</span></div>'
            ...
            """
            st.markdown(html_extrato, unsafe_allow_html=True)

            st.write("")
            if st.button("💾 Salvar Fechamento", use_container_width=True, type="primary"):
                ...
                try:
                    supabase.table("pedidos").update({
                        "valor_frete": tratar_preco(pedido.get('valor_frete', 0)) + novo_frete,
            ...
            st.write("")
            if fone_cliente:
                st.markdown(f'<a href="{link_wpp}" target="_blank" style="background: #25d366; color: white; font-weight: 800; font-size: 13px; border-radius: 10px; padding: 12px; display: flex; justify-content: center; text-decoration: none; box-shadow: 0 4px 10px rgba(37,211,102,0.2);">💬 Enviar Resumo (WhatsApp)</a>', unsafe_allow_html=True)
    ...
    with col_btn2:
        if st.button("✏️ Editar Pedido Inteiro", use_container_width=True):
            st.session_state.modo_edicao_nativo = True
            st.rerun()

# =====================================================
# MODO DE EDIÇÃO INTEGRADO (SUBSTITUI REDIRECIONAMENTO)
# =====================================================
else:
    st.markdown(f"""
    <div class="order-header">
        <div>
            <h1 class="order-id">✏️ Editando Pedido #{id_curto}</h1>
            <span class="order-type-badge {tipo_classe}">{tipo_texto}</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.container(border=True):
        st.markdown("<h4 style='color: #c5721f; margin-top: 0;'>1. Dados do Comprador</h4>", unsafe_allow_html=True)
        c1, c2, c3 = st.columns(3)
        with c1: novo_nome_comp = st.text_input("Nome Comprador", value=pedido.get('cliente_nome', ''))
        with c2: novo_tel_comp = st.text_input("WhatsApp", value=pedido.get('cliente_telefone', ''))
        with c3: novo_cpf = st.text_input("CPF / CNPJ", value=pedido.get('cliente_cpf', ''))

    with st.container(border=True):
        st.markdown("<h4 style='color: #c5721f; margin-top: 0;'>2. Dados da Entrega e Destinatário</h4>", unsafe_allow_html=True)
        c4, c5, c6 = st.columns(3)
        with c4: novo_dest_nome = st.text_input("Nome Destinatário", value=pedido.get('destinatario_nome', ''))
        with c5: novo_dest_tel = st.text_input("Tel Destinatário", value=pedido.get('destinatario_telefone', ''))
        with c6: novo_motivo = st.text_input("Motivo/Ocasião", value=pedido.get('motivo_homenagem', ''))
        
        novo_end = st.text_area("Endereço Completo", value=pedido.get('endereco', ''), height=70)
        
        try: dt_obj = datetime.strptime(str(pedido.get('data_entrega'))[:10], "%Y-%m-%d").date()
        except: dt_obj = date.today()
        
        c7, c8 = st.columns(2)
        with c7: nova_data = st.date_input("Data de Entrega", value=dt_obj, format="DD/MM/YYYY")
        with c8: novo_periodo = st.text_input("Período/Horário", value=pedido.get('periodo_entrega', ''))
        nova_msg = st.text_area("Mensagem do Cartão", value=pedido.get('mensagem', ''), height=70)

    with st.container(border=True):
        st.markdown("<h4 style='color: #c5721f; margin-top: 0;'>3. Alterar Produtos e Adicionais</h4>", unsafe_allow_html=True)
        st.info("💡 Como administrador, você pode reescrever livremente a lista de produtos abaixo.")
        novos_produtos = st.text_area("Lista de Cestas (Uma por linha)", value=pedido.get('produtos', ''), height=100)
        novos_adicionais = st.text_area("Lista de Adicionais / Descontos (Um por linha)", value=pedido.get('adicionais', ''), height=100)
        
        novo_pagamento = st.selectbox("Forma de Pagamento", ["Pix", "Cartão de Crédito", "Faturamento (Boleto)", "Transferência Bancária"], index=["Pix", "Cartão de Crédito", "Faturamento (Boleto)", "Transferência Bancária"].index(pedido.get('pagamento', 'Pix')) if pedido.get('pagamento') in ["Pix", "Cartão de Crédito", "Faturamento (Boleto)", "Transferência Bancária"] else 0)

    st.write("")
    col_salvar, col_cancelar = st.columns(2)
    
    with col_salvar:
        if st.button("💾 Salvar Todos os Dados do Pedido", type="primary", use_container_width=True):
            dados_update = {
                "cliente_nome": novo_nome_comp,
                "cliente_telefone": novo_tel_comp,
                "cliente_cpf": novo_cpf,
