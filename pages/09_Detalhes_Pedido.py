...
def gerar_whatsapp(pedido, adicionais, valor_final, frete_atual, extras_dinamicos_lista, desconto_atual, itens_consulta_catalogo):
    ...
    if float(frete_atual or 0) > 0:
        texto_val += f"🚚 Frete: {formatar_valor(frete_atual)}\n"
    if float(desconto_atual or 0) > 0:
        texto_val += f"🏷️ Desconto: - {formatar_valor(desconto_atual)}\n"
...
def obter_icone_pagamento(metodo):
    ...
    if "pix" in m:
        return '<div class="pgto-badge" style="background: #e6f4ea; border-color: #137333; color: #137333;">⚡ PIX</div>'
    elif "cart" in m:
        return '<div class="pgto-badge" style="background: #e8f0fe; border-color: #1a73e8; color: #1a73e8;">💳 CARTÃO</div>'
    elif "dinheiro" in m:
        return '<div class="pgto-badge" style="background: #fef7e0; border-color: #b06000; color: #b06000;">💵 DINHEIRO</div>'
    elif "transfer" in m:
        return '<div class="pgto-badge" style="background: #f3ece6; border-color: #5a3b28; color: #5a3b28;">🏦 TRANSF.</div>'
    ...
    with st.container(border=True):
        ...
        with aba_cesta:
            ...
            if cesta_selecionada:
                ...
                if config_cesta:
                    ...
                    for grupo in config_cesta:
                        ...
                        with st.container(border=True):
                            ...
                            if maximo == 1:
                                ...
                                escolhido = st.radio("Escolha 1", prods, format_func=lambda p: p["nome"], index=idx_def, key=f"edit_rad_{cat}", label_visibility="collapsed")
                                if escolhido:
                                    selecoes_admin[cat] = [escolhido]
                            else:
                    ...
                    st.info("Esta cesta não possui produtos configurados.")
            
            st.divider()
            col_m1, col_m2 = st.columns(2)
            with col_m1:
                nova_mensagem = st.text_area("💌 Texto do Cartão", value=pedido.get("mensagem") or "", height=120)
            with col_m2:
                novo_endereco = st.text_area("📍 Endereço de Entrega Oficial", value=pedido.get("endereco") or "", height=120)
        ...
        with aba_adicionais:
            ...
            try:
                ...
                if cat_add_id:
                    adicionais_catalogo = supabase.table("produtos").select("*").eq("categoria_id", cat_add_id).execute().data or []
            except:
                pass
            ...
            if adicionais_catalogo:
                ...
                st.caption("O catálogo de adicionais está vazio.")
        ...
        with cs2:
            if st.button("💾 Salvar Informações", use_container_width=True, type="primary"):
                ...
                if "erro_admin" in st.session_state:
                    del st.session_state["erro_admin"]
                try:
                    try:
    ...
    with aba_geral:
        ...
        with st.container(border=True):
            ...
            if endereco_pedido:
                ...
                with col_map2:
                    st.link_button("🚗 Abrir Rota no Waze", url=f"https://waze.com/ul?q={endereco_encoded}&navigate=yes", use_container_width=True)
        ...
        with col_i1:
            with st.container(border=True):
                ...
                if produtos:
                    ...
                    st.caption("Nenhum item configurado.")
                ...
                if adicionais_pedido:
                    for idx_ad, adicional in enumerate(adicionais_pedido):
                        ...
                        if valor is not None:
                            ...
                            st.number_input(f"Definir valor para {nome}", min_value=0.0, value=val_salvo, step=1.0, key=f"cons_{nome}_{idx_ad}", disabled=(bloquear_edicao or pedido_arquivado))
                else:
                    st.caption("Nenhum adicional do catálogo solicitado.")
    ...
    with aba_financeiro:
        ...
        with col_f1:
            with st.container(border=True):
                ...
                with cf1:
                    valor_frete = st.number_input("🚚 Frete (R$)", min_value=0.0, value=float(pedido.get("valor_frete") or 0), step=1.0, disabled=travar_financeiro)
                with cf2:
                    desconto = st.number_input("🏷️ Desconto (R$)", min_value=0.0, value=float(pedido.get("desconto") or 0), step=1.0, disabled=travar_financeiro)
        ...
        with col_f2:
            with st.container(border=True):
                ...
                with col_add1:
                    input_nome_extra = st.text_input("Nome", key="nome_extra_dinamico", disabled=travar_financeiro, placeholder="Ex: Urgência")
                with col_add2:
                    input_val_extra = st.number_input("R$", min_value=0.0, step=1.0, key="val_extra_dinamico", disabled=travar_financeiro)
                with col_add3:
                ...
                if st.session_state["lista_extras_dinamicos"]:
                    ...
                    for idx, extra in enumerate(st.session_state["lista_extras_dinamicos"]):
                        ...
                        with c_n:
                            st.markdown(f"<div style='margin-top: 8px; font-size:12px; font-weight:600;'>{extra['nome']}</div>", unsafe_allow_html=True)
                        with c_v:
                            st.markdown(f"<div style='margin-top: 8px; font-size:12px; font-weight:800; color:#137333;'>{formatar_valor(extra['valor'])}</div>", unsafe_allow_html=True)
                        with c_b:
        ...
        with col_an2:
            with st.container(border=True):
                ...
                if "msg_foto" in st.session_state:
                    if "❌" in st.session_state['msg_foto']:
                        st.error(st.session_state['msg_foto'])
                    else:
                        st.success(st.session_state['msg_foto'])
                ...
                if not pedido_arquivado:
                    ...
                    if novas_fotos:
                        if st.button("📤 Enviar Fotos para a Nuvem", use_container_width=True, type="primary"):
                            ...
                            if sucesso:
                                st.session_state['msg_foto'] = "✅ Arquivos salvos!"
                            else:
                                st.session_state['msg_foto'] = f"❌ Erro: {erro_msg}"
                            st.rerun()
                    st.divider()
                            ...
                            if link_imagem:
                                ...
                                if not pedido_arquivado:
                                    if st.button("🗑️ Deletar", key=f"del_foto_{foto['id']}", use_container_width=True):
                                        ...
                                        if suc:
                                            st.session_state['msg_foto'] = "✅ Foto apagada!"
                                        else:
                                            st.session_state['msg_foto'] = f"❌ Erro: {err_del}"
                                        st.rerun()
                            else:
                                st.caption("⚠️ Link quebrado.")
                else:
                    st.caption("Nenhum anexo ou Polaroid neste pedido.")
        ...
        with col_bot1:
            if st.button("💾 Salvar Ficha Completa", use_container_width=True, type="primary", disabled=bloquear_edicao):
                ...
                for k, v in itens_consulta.items():
                    if v > 0:
                        json_extras_salvar[f"Valor de {k}"] = v
