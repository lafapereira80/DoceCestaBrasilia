...
/* TICKET DE RESUMO INTERNO NA EDIÇÃO */
...
@media print {
    header, footer, section[data-testid="stSidebar"], div[data-testid="stButton"], .st-key-btn_voltar { display: none !important; }
}
</style>
...
# ESTADOS DA SESSÃO PARA CONTROLE DE EDIÇÃO
if "modo_edicao" not in st.session_state:
    st.session_state.modo_edicao = False

if "edit_pedido_id_atual" not in st.session_state or st.session_state.edit_pedido_id_atual != pedido_id:
...
# =====================================================
# MODO DE VISUALIZAÇÃO DA FICHA TÉCNICA (PADRÃO)
# =====================================================
if not st.session_state.modo_edicao:
    ...
    with col2:
        ...
        <div class="finance-row"><span class="info-label">Taxa de Frete:</span><span class="info-value" style="text-align: right;">{formatar_moeda(pedido.get('valor_frete', 0))}</span></div>
        <div class="val-total">TOTAL: {formatar_moeda(pedido.get('valor_total', 0))}</div>
        """, unsafe_allow_html=True)

    st.markdown("</div>", unsafe_allow_html=True)

    # Botões de Ação na Ficha de Visualização
    st.write("")
...
# =====================================================
# MODO DE EDIÇÃO COMPLETA (LAYOUT ESTILO PÁGINA 18)
# =====================================================
else:
    st.markdown(f"""
    <div class="dash-header">
        <div>
            <h1 class="dash-title">Modo de Edição — Pedido #{pedido['id']}</h1>
            <p class="dash-subtitle">Modifique cestas, itens, dados cadastrais e valores livremente ✏️</p>
        </div>
    </div>
    """, unsafe_allow_html=True)
