...
.btn-wpp > a { background: #25d366 !important; color: white !important; font-weight: 800 !important; font-size: 13px !important; border-radius: 8px !important; padding: 12px !important; display: flex; justify-content: center; align-items: center; text-decoration: none !important; }
div[data-testid="stVerticalBlockBorderWrapper"] { background: #ffffff !important; border-radius: 12px !important; border: 1px solid #e8ddd3 !important; padding: 18px !important; margin-bottom: 15px !important; }
</style>
...
if not st.session_state.modo_edicao:
    ...
    with col1:
        ...
        if (pedido.get('pedido_especial') or '').strip():
            ...
            <div style="background:#ede9fe; padding:10px; border-radius:8px; font-size:13px; color:#5b21b6; border-left:3px solid #5b21b6;">{pedido.get('pedido_especial')}</div>"""
        ...
        if pedido.get('adicionais'):
            for linha in pedido.get('adicionais').split("\n"):
                ...
                if linha_limpa and not "EXTRAS E ADICIONAIS" in linha_limpa:
                    if "desconto" in linha_limpa.lower(): html_info2 += f"<div class='item-pill discount'>🔻 {linha_limpa}</div>"
                    else: html_info2 += f"<div class='item-pill'>✨ {linha_limpa}</div>"
        if pedido.get('mensagem'):
            html_info2 += f"""<div class='data-label' style='margin-top: 15px;'>💌 Mensagem do Cartão</div>
            <div style='background:#fdfbf8; padding:10px; border-radius:8px; font-style:italic; font-size:13px; color:#4a2e1b; border-left:3px solid #c5721f;'>"{pedido.get('mensagem')}"</div>"""
    ...
    st.markdown("<div style='font-size: 14px; font-weight: 800; color: #137333; margin-bottom: 8px; margin-top: 15px;'>⚡ Ajustes Rápidos & Valores</div>", unsafe_allow_html=True)
    with st.container(border=True):
        ...
        with c_f4: e_status_rapido = st.selectbox("Status Físico", STATUS_PERMITIDOS, index=st_idx, key="status_rap")
        ...
        if mudou_valores:
            ...
            if tx_id:
                ...
                st.markdown(f"""
                <div style="background:#f0fdf4; border:1px solid #bbf7d0; border-radius:8px; padding:16px; margin:15px 0; display:flex; justify-content:space-between;">
                    <div><div class="data-label" style="color:#166534;">💳 Pagamento</div><div class="data-value" style="color:#15803d; font-size:15px;">♾️ InfinitePay ({tx_id})</div></div>
                    <div><div class="data-label" style="color:#166534;">Horário da Aprovação</div><div class="data-value" style="color:#15803d; font-size:15px;">⏰ {data_f}</div></div>
                </div>
                """, unsafe_allow_html=True)
            
            st.markdown("<div style='font-size: 14px; font-weight: 800; color: #1E293B; margin-bottom: 8px; margin-top: 25px;'>📲 Notificar Cliente</div>", unsafe_allow_html=True)
            ...
            with st.container(border=True):
                ...
                with c_mail:
                    ...
                    with e_col1: email_input = st.text_input("E-mail", value=email_cliente, placeholder="cliente@email.com", label_visibility="collapsed", key="email_dest")
                    with e_col2:
                            ...
                            if email_input:
                                ...
                                if sucesso:
                                    ...
                                    if cliente_id and email_input.strip() != email_cliente:
                                        try: supabase.table("clientes").update({"email": email_input.strip()}).eq("id", cliente_id).execute()
                                        except: pass
    ...
    with c_save1:
        if st.button("💾 SALVAR ALTERAÇÕES COMPLETAS", type="primary", use_container_width=True):
            ...
            try:
                ...
                if cliente_id and e_email.strip() != email_cliente: supabase.table("clientes").update({"email": e_email.strip()}).eq("id", cliente_id).execute()
