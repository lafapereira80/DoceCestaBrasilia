...
/* Estilo para os Containers Nativos do Streamlit */
div[data-testid="stVerticalBlockBorderWrapper"] {
    background: #ffffff !important;
    border: 1px solid #e8ddd3 !important;
    border-radius: 16px !important;
    padding: 24px !important;
    box-shadow: 0 4px 15px rgba(90, 59, 40, 0.02) !important;
...
with st.container(border=True):
    ...
    if st.session_state["itens_orcamento_varejo"]:
        ...
        # ==========================================================
        # FOTOS E POLAROID (DETECTOR FLEXÍVEL NO FINAL DO CARRINHO)
        # ==========================================================
        carrinho_atual_man = st.session_state.get("itens_orcamento_varejo", [])
        termos_foto_man = ["polaroid", "foto", "revelação", "retrato", "imagem"]
        precisa_foto_man = any(any(termo in str(item.get("nome", "")).lower() for termo in termos_foto_man) for item in carrinho_atual_man)
        ...
        if precisa_foto_man:
            ...
            st.markdown("<div style='font-size: 15px; font-weight: 800; color: #d1476a; margin-bottom: 12px;'>📷 Adicionar Fotos (Detectado Polaroid / Foto no carrinho)</div>", unsafe_allow_html=True)
            
            fotos_upload_man = st.file_uploader(
                "Arraste ou selecione os arquivos (Serão enviados ao salvar)", 
                type=["jpg", "jpeg", "png", "webp", "heic"], 
                accept_multiple_files=True, 
                key="uploader_polaroid_manual_bottom",
                label_visibility="collapsed"
            )
            if fotos_upload_man:
                st.success(f"✅ {len(fotos_upload_man)} foto(s) selecionada(s) com sucesso!")
            st.markdown("</div>", unsafe_allow_html=True)
...
with st.container(border=True):
    ...
    if st.button("✅ GRAVAR PEDIDO NO SISTEMA", type="primary", use_container_width=True, disabled=st.session_state.get("man_processando", False)):
        ...
        with st.spinner("Registrando pedido..."):
            ...
            if suc:
                # 1. Correção Nomes das Colunas de Adicionais
                ...
                # 2. Salva as Fotos Polaroid (se houver)
                if 'fotos_upload_man' in locals() and fotos_upload_man:
                    with st.spinner("📦 Enviando fotos Polaroid para o servidor..."):
                        salvar_fotos(p_id, fotos_upload_man)
